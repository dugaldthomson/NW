%% For IABP parsed data, extract a kml file
%Need to load an IABP file in .mat format with a structure named AIS with
%fields named datetime, lat, long and BuoyID
%load('IABP.mat');
%summary(IABP);
%% find each unique BuoyID
[~,contacts] = findgroups(IABP.BuoyID); %count the number of unique IABP contacts by BuoyID

for i = 1:length(contacts)
    rows = IABP.BuoyID==contacts(i);
...
%
%inputs:
%   Name:  name of output file (e.g. 'IABP') (must be a text string)
...
HEADER3=['<styleUrl>#orange-5px</styleUrl>'];
...
% 
% %stats = grpstats(AIS18,'BuoyID');
% 
% [g,contacts] = findgroups(AIS18.BuoyID);  %count the number of unique AIS contacts by BuoyID
% %[g,contacts] = findgroups(AIS.BuoyID);
% 
% 
% %% create a table to send to pwr_kml_tll
% % BuoyID of wave [316024641]
% rows = AIS18.BuoyID==316024641;
% vars = {'datetime','lat','long'};
% WAVE = AIS18(rows,vars);
